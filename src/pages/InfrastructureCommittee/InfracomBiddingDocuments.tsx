...
export default function InfracomBiddingDocuments() {
  ...
  const handleSearch = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    try {
      const res = await axios.post("/infrastructure/searchInfraMasterlist", {
      ...
      if (res.data.status === "success" && res.data.data !== "") {
        ...
        toast({
          title: "Not Found",
          description: "No record with matching project number found.",
          variant: "destructive",
        })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    axios
      .post("/infrastructure/updateBiddingDocuments", formData)
      .then((res) => {
  ...
  return (
    <BaseTemplate>
      ...
      <div className="flex flex-col items-center justify-center h-full w-full ">
        <div className="w-full text-white text-center text-3xl mt-4 mb-4">
          BIDDING DOCUMENTS
        </div>
        <div className="h-full w-[95%] bg-[#274E7E] p-4">
          ...
          >
            ...
            <button
              type="submit"
              className="bg-white text-black p-2 rounded h-8"
            >
              Search
            </button>
          </form>
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-12 w-full flex justify-end">
              <button
                type="submit"
                className="p-2 py-2 bg-white font-semibold rounded mr-4 px-3"
                disabled={!isProjectFound}
              >
                UPDATE
              </button>
              <a
                href="/infrastructure-home"
                className="p-2 py-2 bg-white font-semibold rounded mr-4 px-3"
              >
                CLOSE
              </a>
