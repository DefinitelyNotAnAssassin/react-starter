...
export default function Menu(props) { 
    ...
    return ( 
        <div className = "flex h-[8vh] px-8 w-full items-center justify-between"> 

            <DropdownMenu>
                <DropdownMenuTrigger> <img src = "/hamburger.svg" className = "h-8 w-8" />  </DropdownMenuTrigger>
                <DropdownMenuContent>
            ...
            <div className = "flex gap-4">
                {NAVIGATION_LINKS.map((link) => (
                    <Link to={link.url} key={link.name} className = "text-white"
                    viewTransition>
                        {link.name}
                    </Link>
                ))}
